...
export class GeneratorComponent {
  ...
  async transcode(audios:Blob[]) {
    ...
    console.log(this.ffmpeg.listDir('tmp'));
    ...
    let command = ['-i','output.mp4',"-vf","ass=subtitles.ass:fontsdir=tmp","-c:v","libx264","-preset","ultrafast","-crf","22","-c:a","copy",'outputsub.mp4'];
    ...
    this.videoURL = URL.createObjectURL(new Blob([data.buffer],{type:'video/mp4'}))
      ...
      getSubtitlesAsAss(alignment: string,fontName:string): string {
...
Style: Default,${fontName},22,&Hffffff,&Hffffff,&H0,&H0,0,0,0,0,100,100,0,0,0,0,0,5,0,0,0,0,UTF-8
